...
export async function searchCatalog(params: {
  ...
  if (params.digitized !== undefined)
    searchParams.set("digitized", String(params.digitized));
  if (params.date_start !== undefined)
    searchParams.set("date_start", String(params.date_start));
  if (params.date_end !== undefined)
    searchParams.set("date_end", String(params.date_end));
...
export async function fetchRandomVolume(): Promise<RandomVolume> {
  const res = await fetch(`${API_BASE}/catalog/random`, {
    headers: authHeaders(),
  });
