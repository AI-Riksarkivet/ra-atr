...
<div class="mx-auto max-w-md space-y-6 p-8">
	...
	<!-- Model card -->
	<div
		class="relative rounded-xl bg-card/50 backdrop-blur-sm p-5 space-y-4 {isActive && !modelsReady
			? 'border-glow'
		...
		<div class="space-y-2">
			{#each models as model (model.key)}
				...
				<div class="flex items-center gap-3 py-1.5 {done ? 'opacity-60' : ''}">
					<div
						class="size-7 rounded-lg {done
							? 'bg-success/10 text-success'
							: isActive && pct > 0
								? 'bg-primary/10 text-primary'
								: 'bg-muted text-muted-foreground'} flex items-center justify-center transition-colors"
					>
						<model.icon class="size-3.5" />
					</div>
					<div class="flex-1 min-w-0">
						<div class="flex items-center justify-between">
							<span class="text-sm font-medium {done ? 'text-muted-foreground' : ''}"
								>{t(model.labelKey)}</span
							>
		...
		<div class="rounded-xl border border-destructive/30 bg-destructive/5 p-4 space-y-2">
			...
			{#if is401}
				<p class="text-xs text-muted-foreground">
					Set your HuggingFace token in the browser console:<br /><code
						class="text-xs bg-muted px-1.5 py-0.5 rounded font-mono"
						>sessionStorage.setItem('hf_token', 'hf_...')</code
					>
				</p>
			{/if}
	...
	{:else if !loading && !modelsReady && !autoLoading}
		<Button class="w-full h-11 text-sm font-medium btn-glow rounded-xl" onclick={handleLoad}
