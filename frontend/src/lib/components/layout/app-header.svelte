...
>
	...
	<div class="ml-auto flex items-center gap-1">
		...
		{#if isViewer}
			<Button
				variant={transcriptionOpen ? 'secondary' : 'ghost'}
				size="icon-sm"
				onclick={onToggleTranscription}
				title={t('toolbar.transcriptions')}
			>
		...
		<button
			...
			title={locale() === 'en' ? 'Byt till svenska' : 'Switch to English'}>{locale()}</button
		>
	</div>
</header>
