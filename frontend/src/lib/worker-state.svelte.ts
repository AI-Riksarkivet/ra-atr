...
export class HTRWorkerState {
	...
	onRegionDetected:
		| ((imageId: string, regionId: string, startIndex: number, lines: BBox[]) => number[])
		| null = null;
	onRegionComplete: ((imageId: string, regionId: string) => void) | null = null;
	onLineComplete:
		| ((imageId: string, lineIndex: number, text: string, confidence: number) => void)
		| null = null;
	...
	private createDetectWorker() {
		this.detectWorker = new Worker(new URL('../worker-detect.ts', import.meta.url), {
			type: 'module',
		});
	...
	private createTranscribeWorkers(count: number) {
		...
		for (let i = 0; i < count; i++) {
			const worker = new Worker(new URL('../worker-transcribe.ts', import.meta.url), {
				type: 'module',
			});
	...
	loadModels() {
		...
		if (!this.layoutWorker) {
			this.layoutWorker = new Worker(new URL('../worker-layout.ts', import.meta.url), {
				type: 'module',
			});
