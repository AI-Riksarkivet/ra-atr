    ...
    try {
      ...
      if (statusRes.ok) {
        ...
        const allHealthy = Object.values(
          status.deployments as Record<string, { status: string }>,
...
export interface GpuStatus {
  deployments: Record<
    string,
    { status: string; message?: string; replicas?: number }
  >;
...
async function _ensureUploaded(
...
export async function gpuDetectLayout(
  imageData: ArrayBuffer,
  localId?: string,
): Promise<{
  regions: {
...
export async function gpuDetectLines(
  ...
  lines: { x: number; y: number; w: number; h: number; confidence: number }[];
}> {
...
export async function gpuProcessPage(imageData: ArrayBuffer): Promise<{
  ...
  const res = await fetch(`${baseUrl()}/process-page`, {
    method: "POST",
    body: form,
  });
