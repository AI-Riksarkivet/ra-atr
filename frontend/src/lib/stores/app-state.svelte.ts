...
class AppState {
  ...
  async loadDocumentImage(docId: string) {
    ...
    if (!doc.placeholder) {
      this.imageLoadOrder = [
        docId,
        ...this.imageLoadOrder.filter((id) => id !== docId),
      ];
      return;
    }
  ...
  navigateLine(delta: -1 | 1) {
    ...
    if (current < 0) {
      this.hoveredLine =
        delta === 1 ? allLineIds[0] : allLineIds[allLineIds.length - 1];
    } else {
      ...
      if (idx < 0) {
        ...
        if (nextIdx >= 0 && nextIdx < allLineIds.length)
          this.hoveredLine = allLineIds[nextIdx];
      }
    }
  }
  ...
  populateFromBackend(manifestId: string, groups: TranscriptionGroup[]) {
    for (const group of groups) {
      const doc = this.documents.find(
        (d) =>
          d.manifestId === manifestId && d.pageNumber === group.page_number,
      );
      if (!doc) continue;
  ...
  serializeForSave(): {
    ...
    const docs = this.documents.filter(
      (d) => d.manifestId && d.groups.length > 0,
    );
    ...
    for (const doc of docs) {
      ...
      for (const group of doc.groups) {
        const lines = group.lineIds
          ...
          .filter(
            (line): line is Line =>
              !!line && line.complete && line.text.trim() !== "",
          )
          .map((line, idx) => ({
            line_index: idx,
            bbox: {
  ...
  private async save() {
    ...
    if (!data) {
      console.log("[autosave] skip: no data");
      return;
    }
    ...
    console.log(
      `[autosave] saving ${data.groups.reduce((n, g) => n + g.lines.length, 0)} lines`,
    );
