...
export async function resolveManifestId(referenceCode: string): Promise<string> {
	...
	if (error) {
		throw new Error(
			`Riksarkivet: ${error.textContent || error.getAttribute('code') || 'Unknown error'}`,
		);
	}
	...
	for (const dao of daos) {
		const href =
			dao.getAttribute('xlink:href') ||
			dao.getAttributeNS('http://www.w3.org/1999/xlink', 'href') ||
			'';
...
export async function importVolume(
	...
	try {
		...
		for (let i = 0; i < pages.length; i += concurrency) {
			...
			const results = await Promise.all(batch.map((page) => fetchPageImage(manifestId, page)));
