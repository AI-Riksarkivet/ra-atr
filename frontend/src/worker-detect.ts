...
ort.env.wasm.numThreads = hasSharedBuffer
  ? Math.min(2, Math.max(1, Math.floor(cores / 4)))
  : 1;
...
console.log(
  `[detect] threads: ${ort.env.wasm.numThreads}, SAB: ${hasSharedBuffer}`,
);
  ...
  try {
    switch (e.data.type) {
      case "load_models": {
        ...
        const yoloBytes = await downloadAndCacheModel(
          modelUrl,
          "yolo",
          progress,
          headers,
        ...
        if (DEV)
          console.log(
            "[detect] YOLO loaded, inputs:",
            yoloSession.inputNames,
            "outputs:",
            yoloSession.outputNames,
          );
        self.postMessage({
          type: "model_status",
          payload: { model: "yolo", status: "loaded" },
        });
...
async function processNextRegion() {
  ...
  try {
    ...
    const {
      ...
      padX,
      padY,
    } = preprocessYolo(cropped, 640);
    const yoloTensor = new ort.Tensor("float32", yoloInput, [1, 3, 640, 640]);

    const yoloResult = await yoloSession.run({
      [yoloSession.inputNames[0]]: yoloTensor,
    });
    const yoloOutput = yoloResult[yoloSession.outputNames[0]];
    const protoOutput =
      yoloSession.outputNames.length > 1
