<script lang="ts">
	...
	function handleUpload(files: { name: string; imageData: ArrayBuffer; previewUrl: string }[]) {
		...
		for (let i = 0; i < files.length; i++) {
			const file = files[i];
			const docId = appState.addDocument(
				file.name,
				file.previewUrl,
				file.imageData,
				volumeId,
	...
	function toggleGroup(groupId: string) {
		if (!activeDoc) return;
		activeDoc.groups = activeDoc.groups.map((g) =>
	...
	function handleRiksarkivetResolved(manifestId: string, pages: number[], startPage?: number) {
		...
		if (newPages.length > 0 && backendAvailable) {
			fetchTranscriptions(manifestId)
				.then((groups) => {
	...
	async function handleCatalogLoad(
		...
		metadata?: import('$lib/api').CatalogResult,
		randomPage = false,
	) {
		...
		try {
			const { manifestId, pages } = await resolveVolume(referenceCode, (p) => {
				...
				else if (p.stage === 'manifest')
					catalogLoading = `Found ${p.manifestId}, loading manifest...`;
...
{#if appState.htr.error}
	<div
		class="flex items-center gap-2 bg-destructive/10 px-4 py-2 text-sm text-destructive shrink-0"
	>
		<span class="flex-1">{appState.htr.error}</span>
		<button
			class="text-destructive/60 hover:text-destructive cursor-pointer text-lg leading-none"
			onclick={() => (appState.htr.error = '')}>&times;</button
		>
	</div>
{/if}
{#if catalogLoading}
	<div class="bg-muted px-4 py-1.5 text-xs text-muted-foreground animate-pulse shrink-0">
		{catalogLoading}
	</div>
{/if}
{#if catalogError}
	<div
		class="flex items-center gap-2 bg-destructive/10 px-4 py-1.5 text-xs text-destructive shrink-0"
	>
		<span class="flex-1">{catalogError}</span>
		<button
			class="text-destructive/60 hover:text-destructive cursor-pointer text-sm leading-none"
			onclick={() => (catalogError = '')}>&times;</button
		>
	</div>
{/if}
...
<div class="flex flex-1 overflow-hidden">
	...
	{#if backendAvailable && !leftCollapsed}
		<div class="overflow-hidden border-r border-border flex flex-col" style="width: {leftWidth}%">
			<CatalogPanel
				bind:this={catalogPanel}
				onLoadVolume={(ref, meta) => handleCatalogLoad(ref, meta)}
			/>
		</div>
		<div
	...
	<div class="relative overflow-hidden flex-1">
		...
		{#if activeDoc}
			...
			{#if pageNav}
				{#if pageNav.hasPrev}
					<button
						...
						title="Previous page"><ChevronLeft class="size-5" /></button
					>
				{/if}
				{#if pageNav.hasNext}
					<button
						...
						onclick={() => appState.navigatePage(1)}
						title="Next page"><ChevronRight class="size-5" /></button
					>
				{/if}
			{/if}
			...
			{#if metadataOpen && activeDoc?.manifestId}
				...
				{#if meta}
					<div
						class="absolute bottom-16 left-1/2 -translate-x-1/2 rounded-lg bg-black/50 backdrop-blur-xl border border-white/[0.06] shadow-xl px-4 py-3 text-[0.7rem] text-white/80 leading-relaxed space-y-1 min-w-[280px] max-w-[400px] shadow-lg"
					>
						<div class="font-semibold text-white">{meta.fonds_title}</div>
						{#if meta.creator}<div class="text-white/60">{meta.creator}</div>{/if}
						{#if meta.series_title}<div>
								{meta.series_title}{meta.volume_id ? `, vol. ${meta.volume_id}` : ''}
							</div>{/if}
						{#if meta.date_text}<div class="text-white/50">{meta.date_text}</div>{/if}
						{#if meta.description}<div class="text-white/50 line-clamp-4">
								{meta.description}
							</div>{/if}
			...
			<div
				class="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-3 rounded-xl bg-black/40 backdrop-blur-xl border border-white/[0.06] shadow-2xl shadow-black/40 px-4 py-2 text-[0.7rem] text-white/70"
			>
				{#if activeDoc?.manifestId}
					{@const siblings = appState.documents.filter(
						(d) => d.manifestId === activeDoc.manifestId,
					)}
					{@const meta = appState.volumeMetadata.get(activeDoc.manifestId)}
					<span
						class="font-mono tabular-nums"
						title="Page {activeDoc.pageNumber ?? '?'} of {siblings.length}"
						>{activeDoc.pageNumber ?? '?'}<span class="text-white/40">/{siblings.length}</span
						></span
					>
					{#if meta}
					...
					{/if}
					<button
						class="size-6 rounded-full flex items-center justify-center transition-all cursor-pointer {metadataOpen
							? 'bg-white/30 text-white'
							: 'bg-white/10 text-white/50 hover:text-white hover:bg-white/20'}"
						onclick={() => (metadataOpen = !metadataOpen)}
						title="Volume details"><Info class="size-3" /></button
					>
				{/if}
				{#if totalLines > 0}
					<span class="text-white/20">&middot;</span>
					<span
						class="font-mono tabular-nums"
						title="{completedLines} of {totalLines} lines transcribed"
						>{completedLines}/{totalLines}</span
					>
				{/if}
				{#if appState.htr.pendingLines > 0}
					<span
						class="text-orange-400 font-mono"
						title="{appState.htr.pendingLines} lines currently being transcribed"
						>{appState.htr.pendingLines} in-flight</span
					>
				{/if}
				{#if appState.saving}
					<span class="text-white/40 animate-pulse" title="Saving transcriptions to server"
						>saving</span
					>
				{/if}
				<span class="text-white/20">|</span>
				<button
					class="size-8 rounded-full bg-white/10 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/20 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed {!pageTranscribed &&
					!isRunning
						? 'btn-glow'
						: ''}"
					disabled={isRunning}
					onclick={(e: MouseEvent) => {
						if (activeDoc) {
							if (e.shiftKey && activeDoc.manifestId) {
					...
					title={pageTranscribed
						? 'Re-transcribe page (Shift+click: whole volume)'
						: 'Transcribe page (Shift+click: whole volume)'}
				>
					{#if pageTranscribed}
				...
				<button
					class="size-8 rounded-full flex items-center justify-center transition-all cursor-pointer {appState.selectMode
				...
				>
				<button
					class="size-8 rounded-full flex items-center justify-center transition-all cursor-pointer {showBoxes
						? 'bg-primary text-white ring-2 ring-primary/40'
						: 'bg-white/10 text-white/70 hover:text-white hover:bg-white/20'}"
				...
				>
				<button
					class="size-8 rounded-full flex items-center justify-center transition-all cursor-pointer {showTextOverlay
						? 'bg-primary text-white ring-2 ring-primary/40'
						: 'bg-white/10 text-white/70 hover:text-white hover:bg-white/20'}"
				...
				<div class="relative">
					<button
						class="size-8 rounded-full flex items-center justify-center transition-all cursor-pointer {showFilters
							? 'bg-white/30 text-white'
							: 'bg-white/10 text-white/70 hover:text-white hover:bg-white/20'}"
						onclick={() => (showFilters = !showFilters)}
						title="Image adjustments"><SunMedium class="size-4" /></button
					>
					{#if showFilters}
						<div
							class="absolute bottom-full right-0 mb-2 rounded-lg bg-black/50 backdrop-blur-xl border border-white/[0.06] shadow-xl p-3 space-y-2 min-w-[180px] shadow-lg"
						>
							...
							<input
								...
								min="20"
								max="250"
								bind:value={imageFilters.brightness}
								class="w-full h-1 accent-white/70"
							/>
							<div class="flex items-center justify-between text-[0.65rem] text-white/70">
								<span>Contrast</span>
							...
							<input
								...
								min="20"
								max="300"
								bind:value={imageFilters.contrast}
								class="w-full h-1 accent-white/70"
							/>
							<div class="flex items-center justify-between text-[0.65rem] text-white/70">
							...
							<input
								...
								min="0"
								max="200"
								bind:value={imageFilters.saturate}
								class="w-full h-1 accent-white/70"
							/>
							<button
			...
			<!-- Zoom controls -->
			<div
				class="absolute top-3 right-3 flex flex-col rounded-lg bg-black/40 backdrop-blur-md overflow-hidden"
			>
				<button
					...
					title="Zoom in"><Plus class="size-4" /></button
				>
				<div class="h-px bg-white/10"></div>
				<button
					class="size-8 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 transition-all cursor-pointer"
					onclick={() => docViewer?.resetView()}
					title="Fit to page"><Maximize2 class="size-3.5" /></button
				>
				<div class="h-px bg-white/10"></div>
				<button
					class="size-8 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 transition-all cursor-pointer"
					onclick={() => docViewer?.zoomOut()}
					title="Zoom out"><Minus class="size-4" /></button
				>
				<div class="h-px bg-white/10"></div>
				<button
					class="size-8 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 transition-all cursor-pointer"
					onclick={() => {
						document.querySelector('.relative.overflow-hidden.flex-1')?.requestFullscreen();
						setTimeout(() => docViewer?.resetView(), 100);
					}}
					title="Fullscreen"><Maximize class="size-3.5" /></button
				>
				<div class="h-px bg-white/10"></div>
				<button
					class="size-8 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 transition-all cursor-pointer"
					...
					title="Print page"><Printer class="size-3.5" /></button
				>
			</div>
		{:else}
			...
			<TranscriptionPanel
				...
				onFocusGroup={(lineIds, rect) => {
					if (lineIds.length > 0) {
						...
						const bboxes = lineIds
							.map((id) => activeDoc?.lines.find((l) => l.id === id)?.bbox)
							.filter(Boolean);
						if (bboxes.length > 0) {
							let minX = Infinity,
								minY = Infinity,
