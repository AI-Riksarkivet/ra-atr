  ...
  try {
    switch (e.data.type) {
      case "load_model": {
        ...
        const bytes = await downloadAndCacheModel(
          modelUrl,
          "layout",
          progress,
          headers,
        ...
        self.postMessage({
          type: "model_status",
          payload: { model: "layout", status: "loaded" },
        });
      ...
      case "detect_layout": {
        ...
        const imageTensor = new ort.Tensor("float32", chw, [
          1,
          3,
          INPUT_SIZE,
          INPUT_SIZE,
        ...
        if (DEV) {
          console.log(
            `[layout] ${regions.length} regions detected, image ${origW}x${origH}`,
          );
          for (const r of regions.slice(0, 5)) {
            console.log(
              `[layout]   ${r.label} ${(r.confidence * 100).toFixed(0)}% [${r.x.toFixed(0)},${r.y.toFixed(0)} ${r.w.toFixed(0)}x${r.h.toFixed(0)}]`,
            );
          }
        }
        self.postMessage({
          type: "layout_result",
          payload: { imageId, regions },
        });
        break;
      }
    }
