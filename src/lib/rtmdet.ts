...
function nms(
  boxes: { x1: number; y1: number; x2: number; y2: number; score: number; idx: number }[],
  iouThreshold: number,
): number[] {
  ...
  for (const box of sorted) {
    ...
    for (const keptIdx of keep) {
      ...
      if (iou > iouThreshold) { dominated = true; break; }
    }
...
export function parseRTMDetOutput(
  ...
  const candidates: { x1: number; y1: number; x2: number; y2: number; score: number; idx: number }[] = [];
