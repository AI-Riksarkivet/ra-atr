...
export function exportJSON(doc: ImageDocument): string {
  const lines = doc.lines.filter(l => l.complete).map(l => ({
    text: l.text,
...
export function exportPageXML(doc: ImageDocument): string {
  ...
  const regionBlocks: { rect: { x: number; y: number; w: number; h: number }; lines: Line[] }[] = [];
    ...
    if (groupLines.length > 0) {
      regionBlocks.push({
        rect: group.rect ?? {
          ...
          w: Math.max(...groupLines.map(l => l.bbox.x + l.bbox.w)) - Math.min(...groupLines.map(l => l.bbox.x)),
          h: Math.max(...groupLines.map(l => l.bbox.y + l.bbox.h)) - Math.min(...groupLines.map(l => l.bbox.y)),
        },
        lines: groupLines,
      });
    }
  ...
  if (regionBlocks.length === 0 && lines.length > 0) {
    regionBlocks.push({
      rect: {
        ...
        w: Math.max(...lines.map(l => l.bbox.x + l.bbox.w)) - Math.min(...lines.map(l => l.bbox.x)),
        h: Math.max(...lines.map(l => l.bbox.y + l.bbox.h)) - Math.min(...lines.map(l => l.bbox.y)),
      },
      lines,
    });
  }
  ...
  const regions = regionBlocks.map((region, ri) => {
    const textLines = region.lines.map((l, li) => {
